import { chmodSync, mkdtempSync, renameSync, rmSync, symlinkSync, truncateSync, writeFileSync } from "node:fs";
...
function directory(): string { const path = mkdtempSync(join(tmpdir(), "agent-bridge-archive-file-")); directories.push(path); return path; }
...
function archiveFile(bytes: Buffer): string {
  const path = join(directory(), "archive.ndjson"); writeFileSync(path, bytes, { mode: 0o600 }); chmodSync(path, 0o600); return path;
}
...
describe("PortableArchiveFile", () => {
  ...
  it("refuses to prove durability after the verified pathname is replaced", () => {
    ...
    renameSync(path, `${path}.moved`); writeFileSync(path, bytes, { mode: 0o600 });
  ...
  it("rejects oversized sparse files, symlinks, and forged receipt eligibility", () => {
    const root = directory(); const huge = join(root, "huge.ndjson"); writeFileSync(huge, "x", { mode: 0o600 }); truncateSync(huge, PORTABLE_ARCHIVE_MAX_BYTES + 1);
    expect(() => new PortableArchiveFile(huge)).toThrow(/size/);
    const valid = join(root, "valid.ndjson"); writeFileSync(valid, encode({ workspace: "acme", messages: [message()], receipts: [] }), { mode: 0o600 });
    ...
    const forgedPath = join(root, "forged.ndjson"); writeFileSync(forgedPath, text, { mode: 0o600 });
