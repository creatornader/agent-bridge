...
- `src/postgres-production-preflight.ts`: Read-only live PostgreSQL migration preflight
...
- Managed metadata version 2 records a versioned stdio or Streamable HTTP target while retaining the released launch projection used by bounded registration proofs. Version 1 stdio metadata remains readable and upgrades on update. Native HTTP targets are limited to explicit loopback HTTP URLs with a port and `/mcp` path. Desktop endpoint targets remain exact stdio proxy launches whose arguments contain that loopback URL. Updates retain the private backend and existing v4 journal, resume, and rollback boundaries.
...
- Migrations use the schema-owner `AGENT_BRIDGE_DATABASE_URL`. The gateway requires a restricted `AGENT_BRIDGE_RUNTIME_DATABASE_URL` and never runs migrations at startup. Production hosts may supply `AGENT_BRIDGE_RUNTIME_DATABASE_CA_BASE64`; the gateway decodes that CA in memory, removes `sslmode` and `sslrootcert` from the URL, and forces certificate and hostname verification. It rejects the `ssl` parameter and client-certificate file parameters with that override.
...
- Native DR preserves one complete local SQLite authority or PostgreSQL authority in a private framed bundle. It does not translate between providers, and it rejects gateway edge SQLite files. PostgreSQL backups combine a schema-only custom dump with canonical role, membership, and default-ACL state. A database-derived `NOLOGIN` backup-reader grants only the schema owner read access to every Agent Bridge table and sequence. A native backup source requires the schema owner to be a true superuser or hold `BYPASSRLS`; `CREATEROLE` alone remains sufficient for migrations. A target restore superuser can validate the intentionally suspended restored schema-owner shell. A restricted runtime login cannot substitute for that authority. Restore requires the same database name and major, a dedicated fresh target, superuser authority, exact-major tools, and explicit acceptance of source SQL risk. Claimed deliveries become retryable work. External principals remain `NOLOGIN`; runtime memberships are omitted; active control and archive memberships are omitted and receive explicit revocation events. Restore may reuse a preexisting schema-owner role only when it is the target session user and a true superuser. Partial restore failure disables target connections and reports residue. PostgreSQL source and target URLs come only from `AGENT_BRIDGE_DR_SOURCE_DATABASE_URL` and `AGENT_BRIDGE_DR_TARGET_DATABASE_URL`.
- PostgreSQL control and archive readiness accept a true-superuser schema owner or a managed-provider schema owner with `CREATEROLE`. PostgreSQL 16 and newer may split schema-owner admin authority from inherit and set authority across the bootstrap grant and a self-grant. Native DR may restore the same authority as one combined bootstrap-superuser grant. Readiness aggregates effective options while validating each allowed raw shape. Registered principals must still be non-superuser logins with the exact direct membership graph.
