...
- Managed-client operations use owner-private revisioned manifests under `~/.agent-bridge/operations/<uuid>`, private snapshots, and locks keyed by runtime plus stable instance. Sensitive file access pins its immediate directory; stale-lock recovery pins the operation root and locks directory throughout recovery. Each immutable ordered step records target kind, non-sensitive locator, unique snapshot artifact, before/after digests, durable pre-write intent, and post-verification observed-applied state. All verified before-state snapshots must be durable before mutation starts, and snapshot publication never replaces residue. Restart classification identifies the exact pending step: before-state retries, after-state advances only after durable intent, and every other state blocks as ambiguous. Stale recovery requires same-host PID-death proof. Linked, replaced, corrupt, oversized, contradictory, or ambiguous state fails closed; inspection exposes safe summaries only. Public mutating lifecycle commands remain blocked until terminal snapshot cleanup is implemented. Active and ambiguous operations retain snapshots. Terminal cleanup must verify and unlink artifacts, sync the directory where supported, report uncertain durability, avoid promises of physical erasure, and rotate credentials when retained rollback copies contained them.
