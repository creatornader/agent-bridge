...
- Migration 018 adds two gateway-only HTTP 2.1 endpoint-migration challenge operations. Both use active bound request authority, the canonical credential and operation rate buckets, and a direct active issuer-to-successor credential link. PostgreSQL stores only a domain-separated challenge commitment for at most 60 seconds. Runtime access is limited to the two challenge functions. Endpoint and owner-control v5 attestations seal the catalog. A consumed challenge does not authorize endpoint cutover.
