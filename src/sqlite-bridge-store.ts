...
function message(row: Row): BridgeMessage { const policy=parse(row.delivery_policy);if(policy?.notBefore==null)delete policy?.notBefore;return { id: String(row.id), workspace: String(row.workspace), project: row.project == null ? undefined : String(row.project), source: String(row.source), sequence: String(row.sequence), type: String(row.type), content: String(row.content), contentType: String(row.content_type), data: parse(row.data), targets: parse(row.targets) ?? [], threadId: row.thread_id as string | undefined, replyToId: row.reply_to_id as string | undefined, correlationId: row.correlation_id as string | undefined, causationId: row.causation_id as string | undefined, priority: row.priority as BridgeMessage["priority"], expiresAt: row.expires_at as string | undefined, idempotencyKey: row.idempotency_key as string | undefined, atribReceiptId: row.atrib_receipt_id as string | undefined, informedBy: parse(row.informed_by), metadata: parse(row.metadata), deliveryPolicy: policy, createdAt: String(row.created_at) }; }
function delivery(row: Row): BridgeDelivery { return { id: String(row.id), messageId: String(row.message_id), workspace: String(row.workspace), recipient: String(row.recipient), state: row.state as BridgeDelivery["state"], attempt: Number(row.attempt), cycleAttempt: Number(row.cycle_attempt), requeueCount: Number(row.requeue_count), createdAt: String(row.created_at), priorityRank: Number(row.priority_rank), availableAt: String(row.available_at), leaseToken: row.lease_token as string | undefined, leaseOwner: row.lease_owner as string | undefined, leaseExpiresAt: row.lease_expires_at as string | undefined, lastError: row.last_error as string | undefined, lastActor: row.last_actor as string | undefined, lastAction: row.last_action as BridgeDelivery["lastAction"] }; }
