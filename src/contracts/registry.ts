...
export const operations: readonly OperationContract[] = [
  operation({ id: "capabilities", summary: "Discover operations available on the current surface.", request: Empty, response: Type.Object({ protocolVersion: Type.String(), currentProtocolVersion: Type.String(), selectedProtocolVersion: Type.String(), supportedProtocolVersions: StringArray, scopeEnforcement: Type.Boolean(), requestAuthority: Type.Boolean(), rowIsolation: Type.Boolean(), authorizationModel: Type.Enum(["scoped-credential", "credential-wide", "process-identity"]), surface: Type.Enum(["mcp", "http", "cli"]), provider: Type.Enum(["local", "gateway"]), implementationVersion: Type.Optional(Type.String()), implementationRevision: Type.Optional(Type.String({ pattern: "^[0-9a-f]{7,40}$" })), grantedScopes: Type.Optional(StringArray), operations: Type.Array(Type.Object({ id: Type.String(), summary: Type.String(), requiredScopes: StringArray, mcp: Type.Optional(Type.Any()), http: Type.Optional(Type.Any()), cli: Type.Optional(Type.Any()) }, { additionalProperties: true })) }, { additionalProperties: true }), scopes: [], providers: ALL_PROVIDERS, mcp: { name: "capabilities" }, http: { method: "GET", path: "/v2/capabilities" }, cli: { command: "capabilities", options: [] } }),
  ...
  operation({ id: "claim_delivery", summary: "Atomically claim the next targeted delivery, or one eligible message when requested.", request: Type.Object({ leaseMs: OptionalNumber(), messageId: Type.Optional(Type.String({ pattern: UUID_PATTERN })), maxAttempts: OptionalNumber() }, { additionalProperties: false }), response: ClaimResponseSchema, scopes: ["deliveries:claim"], providers: DELIVERY_PROVIDERS, mcp: { name: "claim" }, http: { method: "POST", path: "/v2/deliveries/claim" }, cli: { command: "claim", options: ["lease-ms", "message-id", "max-attempts"], response: Type.Union([Type.Object({ delivery: DeliverySchema, leaseToken: Type.String() }, { additionalProperties: true }), Type.Null()]) } }),
...
export function validateRequestForProtocol(operationId: string, value: unknown, protocolVersion: string): Record<string, unknown> {
  if (protocolVersion === LEGACY_PROTOCOL_VERSION) {
    ...
    if (operationId === "claim_delivery" && value && typeof value === "object"
      && (value as Record<string, unknown>).messageId !== undefined) {
      throw new ContractValidationError(operationId, [{ path: "/messageId", message: `Field is not supported by protocol ${LEGACY_PROTOCOL_VERSION}` }]);
    }
  }
...
export function capabilityDocument(context: { surface: ContractSurface; provider: ContractProvider; selectedProtocolVersion?: string; requestAuthority?: boolean; rowIsolation?: boolean; implementationVersion?: string; implementationRevision?: string } = { surface: "http", provider: "gateway" }) {
  ...
  return {
    ...
    ...(context.implementationVersion ? { implementationVersion: context.implementationVersion } : {}),
    ...(context.implementationRevision ? { implementationRevision: context.implementationRevision } : {}),
