...
export class HttpBridgeStore implements BridgeStore {
  ...
  async initialize(): Promise<void> {
    await this.request("/readyz", { authenticated: false });
  ...
  private async request(path: string, options: { method?: string; body?: unknown; authenticated?: boolean } = {}): Promise<any> {
    try {
  ...
  async insertMessage(message: Omit<BridgeMessage, "sequence" | "createdAt">): Promise<InsertMessageResult> {
    ...
    return this.request("/v2/messages", { method: "POST", body: draft });
  }
  async listMessages(principal: BridgePrincipal, query: MessageQuery = {}): Promise<MessagePage> {
    ...
    return this.request(`/v2/history${params.size ? `?${params}` : ""}`);
  }
