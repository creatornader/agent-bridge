...
export class PostgresCredentialResolver implements CredentialResolver {
  ...
  async resolve(token: string, signal?: AbortSignal): Promise<AuthenticatedCredential | null> {
    ...
    const result = await this.db.query<{
      id: string; workspace_id: string; principal: string;
    }>(
      `SELECT credential.id, credential.workspace_id, agent.principal
    ...
    if (signal?.aborted) throw signal.reason;
    return row ? { id: row.id, principal: { workspace: row.workspace_id, agent: row.principal } } : null;
  }
}
